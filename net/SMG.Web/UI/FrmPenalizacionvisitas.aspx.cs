...
{
    public partial class  FrmPenalizacionvisitas : FrmBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                if (rdbBusquedas.Items[0].Selected || rdbBusquedas.Items[1].Selected)
                {
        ...
        protected void CargarDatosBusqueda(Int64 NumPagina)
        {
            try
            {
                ...
                switch (rdbBusquedas.SelectedIndex)
                {
                    case 0:
                        //BONIFICABLES.
                        drExcel = Consultas.ObtenerVisitasPenalizablesBonificables(proveedor,1);
                        dtExcel.Load(drExcel);
                        lblNumEncontrados.Text = dtExcel.Rows.Count.ToString();

                        datos = null;
                        dt = new DataTable();
                        datos = Consultas.ObtenerVisitasPenalizablesBonificablesPorPagina(proveedor, (NumPagina - 1) * 50, ((NumPagina - 1) * 50) + 50,1);// Consultas.ObtenerSolicitudesBonificables(proveedor);
                        dt.Load(datos);
                        
                        break;
                    case 1:
                        //PENALIZABLES.
                        drExcel = Consultas.ObtenerVisitasPenalizablesBonificables(proveedor, 0);
                        dtExcel.Load(drExcel);
                        lblNumEncontrados.Text = dtExcel.Rows.Count.ToString();

                        datos = null;
                        dt = new DataTable();
                        datos = Consultas.ObtenerVisitasPenalizablesBonificablesPorPagina(proveedor, (NumPagina - 1) * 50, ((NumPagina - 1) * 50) + 50,0);// Consultas.ObtenerSolicitudesBonificables(proveedor);
                        dt.Load(datos);
                    ...
                    case -1:
                        switch (rdbBusquedas0.SelectedIndex)
                        {
                            case 0:
                                ...
                                drExcel = Consultas.ObtenerVisitasAPenalizaBonificar(proveedor);
                                dtExcel.Load(drExcel);
                                lblNumEncontrados.Text = dtExcel.Rows.Count.ToString();

                                datos = null;
                                dt = new DataTable();
                                datos = Consultas.ObtenerVisitasAPenalizaBonificarPorPagina(proveedor, (NumPagina - 1) * 50, ((NumPagina - 1) * 50) + 50);// Consultas.ObtenerSolicitudesBonificables(proveedor);
                                dt.Load(datos);
                                break;
                            case 1:
        ...
        {
            for (int i = 0; i < grdListado.Rows.Count; i++)
            {
                ...
                if (Chk.Checked == true)
                {
                    ...
                    Int32 Id = Int32.Parse(HdnID.Value.ToString());//Int32.Parse(grdFacturas.Rows[i].Cells[26].Text);
                    AppPrincipal usuarioPrincipal = (AppPrincipal)CurrentSession.GetMandatoryAttribute(CurrentSession.SESSION_DATOS_USUARIO);
