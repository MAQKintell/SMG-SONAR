...
namespace Iberdrola.SMG.UI
{
    public partial class FrmFacturacionRevisionPrecinte : FrmBaseListado
    {
        ...
        protected void btnRealizarConsulta_Click(object sender, EventArgs e)
        {
            ...
            if (rdbBusquedas0.Items[0].Selected || rdbBusquedas.Items[0].Selected)
            {
                CargarDatos(intPageIndex, false);
            }
            else
            {
                CargarDatos(intPageIndex, true);
            }
        ...
        protected void CargarDatos(int NumPagina,Boolean Penalizable)
        {
            try
            {
                IDataReader drDatos;
                ...
                if (NumPagina == 1)
                {
                    if (Penalizable == false)
                    {
                        drExcel = Consultas.ObtenerSolicitudesRevisionPrecinteRealizadas(Proveedor);
                    }
                    else
                    {
                        drExcel = Consultas.ObtenerSolicitudesRevisionPrecintePenalizables(Proveedor);
                    }
                ...
                {
                    drDatos = Consultas.ObtenerSolicitudesRevisionPrecintePenalizablesPorPagina(Proveedor, (NumPagina - 1) * 50, ((NumPagina - 1) * 50) + 50);
                }
                dt.Load(drDatos);
                ...
                CurrentSession.SetAttribute(CurrentSession.SESSION_TITULO_DATOS_EXCEL, "SOLICITUDES PRECINTE");
        ...
        {
            //rdbBusquedas
            //Esto esta mal hay que controlar si se busca las penalizables o bonifcables.
        ...
        {
            //if (rdbBusquedas.Items[0].Selected)
            //{
            //    Bonificar();
            //}
            //else if (rdbBusquedas.Items[1].Selected)
            //{
            //    Penalizar();
            //}
        }
